# Embers of November — Localization Style Guide
...
The player may choose Marianne, Tadeusz, or Antoni at game start. Some shared system strings (companion prompts, tooltips referring to "the protagonist") need gender-aware variants. The source has `{PRONOUN_HE_SHE}`, `{ARTICLE_NOM_M_F}`, etc. tags for these. Do not flatten these; they expand to character-appropriate forms at runtime. See the engine's localization placeholder reference (`/loc/placeholders.md`) for the full set.

### 9.3 Plural and case-aware placeholders

Numeric strings use ICU MessageFormat plurals: `{count, plural, one {# kula} few {# kule} many {# kul} other {# kuli}}`. Translators must populate all four Polish plural categories where they apply. The build pipeline rejects strings missing categories.
